...
def get_vacancies_filtered_by_name(
    vacancies: list[Vacancy], search_query: str
) -> list[Vacancy]:
    """Возвращает список вакансий, отфильтрованный по наименовании вакансии."""
    filtered_list = [
        vacancy for vacancy in vacancies if search_query in vacancy.get("name")
    ]
    return filtered_list


def get_vacancies_filtered_by_salary_from(
    vacancies: list[Vacancy], salary_from: int, salary_to: int
) -> list[Vacancy]:
    ...
    for v in vacancies:
        if v.get("salary") is None:
            v["salary"] = int(0)
    vacancies_list = [
        v for v in vacancies if salary_from <= v.get("salary") <= salary_to
    ]
